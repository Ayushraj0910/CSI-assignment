...
`app.py` is a Python script, not a program you double-click — it needs to
be run from a terminal with Python. Here's the full path from zero:
...
1. Ingest the 3 files in `sample_data/` (a resume, a research paper, and
   meeting notes — deliberately different document types to exercise the
   ingestion module across domains).
2. Chunk, embed, and index them.
3. Run 6 validation questions (5 in-domain, 1 out-of-domain control) and
   print each grounded answer with its cited source(s) and timing,
   directly in your terminal.
4. Regenerate two report files:
   - `logs/validation_log.md` — per-question answers, cited sources, and
     the top retrieved chunks with their vector/keyword/fused scores
   - `metrics_report.md` — chunking profile, embedding dimensions,
     vector store choice, generation backend, and timing breakdown
...
Instead of the built-in demo questions in `app.py`, open a Python shell
(or a new `.py` file) in the same folder and use the pipeline directly:
