...
validate.py
===========
Fixed-batch validation runner for the Document Question Answering (RAG) system.

This runs a set of pre-written test questions (5 in-domain + 1 out-of-domain
control) against the bundled sample_data/ corpus and writes:
  - logs/validation_log.md  (answers, sources, retrieved chunks, latency)
  - metrics_report.md       (chunking/embedding/vector-store/LLM configuration)

Use this to (re)generate the evaluation artifacts. If you just want to ask
your own questions about your own documents interactively, use `app.py`
instead:
